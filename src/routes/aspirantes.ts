...
import {ZodAspiranteObj, ZodAspiranteIdObj,ZodAspiranteIdObjUpdate} from '@/validation/ZodAspirante'
...
router.put('/status', async (req: Request, res: Response) => {
    try{
        const camposValidados = ZodAspiranteIdObjUpdate.parse(req.body)
        let aspiranteBusqueda = await prisma.aspirantes.findFirst({
            where : {id: camposValidados.id},
        });
